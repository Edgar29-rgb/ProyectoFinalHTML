	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$("a.change-password").click(function() {
				var password = prompt("Por favor escribe su nueva contraseña:");
			...
			$(".dropdown .new-file").click(function() {
				...
				if (path.length > 0) {
					var name = prompt("Por favor ingrese el nuevo nombre:", "new-file.php"),
					...
					alertBox("Warning", "Seleccione el archivo en el directorio", "yellow");
				}
			});
			...
			$(".dropdown .new-dir").click(function() {
				...
				if (path.length > 0) {
					var name = prompt("Escriba el nombre:", "new-dir"),
						end = path.substring(path.length - 1),
						dir = "";
			...
			$(".dropdown .save").click(function() {
				...
				if (path.length > 0) {
					...
					alertBox("Warning", "Seleccionar archivo", "yellow");
				}
			});
			...
			$(".dropdown .delete").click(function() {
				...
				if (path.length > 0) {
					if (confirm("Estas seguro en eliminar el archivo?")) {
						$.post("<?= $_SERVER['PHP_SELF'] ?>", {
							action: "delete",
							path: path
						}, function(data) {
					...
					alertBox("Warning", "Seleccionar archivo en el directorio", "yellow");
				}
			});
			...
			$(".dropdown .rename").click(function() {
				...
				if (path.length > 0) {
					...
					var name = prompt("agregar nuevo nombre:", new_file_name);
					...
					alertBox("Warning", "Seleccinar archivo en el directorio", "yellow");
				}
			});
			...
			$(window).on("hashchange", function() {
				...
				if (hash.length > 0) {
					...
					} else if (confirm("Desctar cambios?")) {
						$("#digest").val("");
...
<body>
	...
	<div class="container-fluid">
		...
		<div class="row p-3">
			<div class="col-md-3">
				<h1><a href="">ProyectoFinal</a></h1>
			</div>
			<div class="col-md-9">
				<div class="float-left">
					<div class="dropdown float-left">
						<button class="btn btn-secondary" type="button" onclick="javascript:window.print()">Imprimir</button>
						<button class="btn btn-secondary dropdown-toggle" type="button" id="fileMenu" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Archivo</button>
						<div class="dropdown-menu" aria-labelledby="fileMenu">
							<?php if (in_array('newfile', $permissions)) { ?>
								<a class="dropdown-item new-file" href="javascript:void(0);">Nuevo archivo <span class="float-right text-secondary">N</span></a>
							<?php } ?>

							<?php if (in_array('newdir', $permissions)) { ?>
								<a class="dropdown-item new-dir" href="javascript:void(0);">Nuevo directorio</a>
							<?php } ?>

							<?php if (in_array('uploadfile', $permissions)) { ?>
								<a class="dropdown-item upload-file" href="javascript:void(0);">Subir archivo <span class="float-right text-secondary">U</span></a>
							<?php } ?>
							...
							<?php if (in_array('newfile', $permissions) || in_array('editfile', $permissions)) { ?>
								<a class="dropdown-item save disabled" href="javascript:void(0);">Guardar <span class="float-right text-secondary">S</span></a>
							<?php } ?>

							<?php if (in_array('deletefile', $permissions) || in_array('deletedir', $permissions)) { ?>
								<a class="dropdown-item delete disabled" href="javascript:void(0);">Eliminar <span class="float-right text-secondary">D</span></a>
							<?php } ?>

							<?php if (in_array('renamefile', $permissions) || in_array('renamedir', $permissions)) { ?>
								<a class="dropdown-item rename disabled" href="javascript:void(0);">Renombrar <span class="float-right text-secondary">R</span></a>
							<?php } ?>

							<a class="dropdown-item reopen disabled" href="javascript:void(0);">Re-abrir <span class="float-right text-secondary">O</span></a>
							<div class="dropdown-divider"></div>
							<a class="dropdown-item close disabled" href="javascript:void(0);">Cerrar <span class="float-right text-secondary">C</span></a>
						</div>
					</div>
